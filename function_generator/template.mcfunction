...
execute as @r at @s as @e[type=item,distance=..6,nbt={OnGround:1b,Item:{id:"[$ingredient_1_name]",Count:[$ingredient_1_count]b}}] at @s run tag @s add [$ingredient_1_output_name]
execute as @r at @s as @e[type=item,distance=..6,nbt={OnGround:1b,Item:{id:"[$ingredient_2_name]",Count:[$ingredient_2_count]b}}] at @s run tag @s add [$ingredient_2_output_name]
...
execute as @e[type=item,tag=craft_event] at @s run data merge entity @s {Motion:[0.0,0.3,0.0],Tags:[],Item:{id:"minecraft:enchanted_book",Count:1b,tag:{StoredEnchantments:[{id:"[$stored_enchantments_id]",lvl:[$stored_enchantments_lvl]}]}}}
