...
execute as @e[tag=drop_obsidian] at @s if entity @e[tag=drop_experience_bottle,distance=..1] run tag @s add drop_makedust
execute as @e[tag=drop_writable_book] at @s if entity @e[tag=drop_makedust,distance=..1] run tag @s add craft_event

execute as @e[type=item,tag=craft_event] at @s run kill @e[tag=drop_experience_bottle,distance=..1,limit=1]
execute as @e[type=item,tag=craft_event] at @s run kill @e[tag=drop_makedust,distance=..1,limit=1]
execute as @e[type=item,tag=craft_event] at @s run playsound minecraft:entity.experience_orb.pickup master @a
execute as @e[type=item,tag=craft_event] at @s run data merge entity @s {Motion:[0.0,0.3,0.0],Tags:[],Item:{id:"minecraft:enchanted_book",Count:1b,tag:{StoredEnchantments:[{id:"minecraft:unbreaking",lvl:3}],display:{Name:"{\"text\":\"Magic Dust\",\"color\":\"dark purple\",\"bold\":false,\"italic\":false}",Lore:["The Sorcerer's Tool."]},Unbreakable:1b,Damage:2}}}
