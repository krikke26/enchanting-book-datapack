...
execute as @r at @s as @e[type=item,distance=..6,nbt={OnGround:1b,Item:{id:"minecraft:water_bucket",Count:1b}}] at @s run tag @s add drop_water_bucket_depth_strider_1
...
execute as @e[tag=drop_water_bucket_depth_strider_1] at @s if entity @e[tag=drop_lapis_lazuli_depth_strider_1,distance=..1] run tag @s add drop_enchanted_book_depth_strider_1
