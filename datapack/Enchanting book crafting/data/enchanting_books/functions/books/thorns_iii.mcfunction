...
execute as @r at @s as @e[type=item,distance=..6,nbt={OnGround:1b,Item:{id:"minecraft:double_plant",Count:24b}}] at @s run tag @s add drop_double_plant_thorns_3
...
execute as @e[tag=drop_double_plant_thorns_3] at @s if entity @e[tag=drop_lapis_lazuli_thorns_3,distance=..1] run tag @s add drop_enchanted_book_thorns_3
