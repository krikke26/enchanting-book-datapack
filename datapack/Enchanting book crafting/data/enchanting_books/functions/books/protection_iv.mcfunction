...
execute as @r at @s as @e[type=item,distance=..6,nbt={OnGround:1b,Item:{id:"minecraft:iron_ingot",Count:32b}}] at @s run tag @s add drop_iron_ingot
...
execute as @e[type=item,tag=craft_event] at @s run data merge entity @s {Motion:[0.0,0.3,0.0],Tags:[],Item:{id:"minecraft:enchanted_book",Count:1b,tag:{StoredEnchantments:[{id:"minecraft:protection",lvl:4}]}}}
