...
execute as @r at @s as @e[type=item,distance=..6,nbt={OnGround:1b,Item:{id:"minecraft:frosted_ice",Count:16b}}] at @s run tag @s add drop_frosted_ice_frost_walker_2
...
execute as @e[tag=drop_frosted_ice_frost_walker_2] at @s if entity @e[tag=drop_lapis_lazuli_frost_walker_2,distance=..1] run tag @s add drop_enchanted_book_frost_walker_2
